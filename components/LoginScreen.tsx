...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLoginSuccess, users, systemConfig }) => {
      ...
      if (systemConfig.ipCheckEnabled) {
          ...
          fetch('https://api.ipify.org?format=json')
              ...
              .catch(err => {
                  ...
                  // Fail safe: If we can't get IP, assume allowed? Or blocked? 
                  // For security, usually blocked, but for usability here, maybe allow with warning?
  ...
  const checkIpAgainstWhitelist = (ip: string) => {
      ...
      const allowed = systemConfig.allowedIPs.some(pattern => {
          if (pattern.includes('*')) {
              // Wildcard match (e.g. 192.168.1.* -> matches 192.168.1.5)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (systemConfig.ipCheckEnabled && !isIpAllowed) {
        // Allow Admin to bypass IP check to prevent lockout?
        // Check if user is admin FIRST (credentials check)
        ...
        <form className="mt-6 sm:mt-8 space-y-6" onSubmit={handleSubmit}>
          <div className="rounded-md shadow-sm -space-y-px">
            <div>
              ...
              <input
                ...
                required
                autoFocus
                className={`appearance-none rounded-none rounded-t-md relative block w-full px-3 py-3 border placeholder-gray-500 text-white bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-lime-500 focus:z-10 text-base sm:text-sm ${error ? 'border-red-500' : 'border-gray-600'}`}
            ...
            <div>
              ...
              <input
                ...
                required
                className={`appearance-none rounded-none rounded-b-md relative block w-full px-3 py-3 border placeholder-gray-500 text-white bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-lime-500 focus:z-10 text-base sm:text-sm ${error ? 'border-red-500' : 'border-gray-600'}`}
          ...
          <div>
            <button
              ...
              className={`group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-md text-white transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-lime-500 
                  ${systemConfig.ipCheckEnabled && !isIpAllowed 
                    ? 'bg-gray-600 cursor-not-allowed opacity-50' 
                    : 'bg-lime-500 hover:bg-lime-600 shadow-[0_0_15px_rgba(132,204,22,0.4)]'
                  }`}
            >
              {t('login_btn')}
            </button>
