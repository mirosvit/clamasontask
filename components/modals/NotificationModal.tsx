      ...
      <div className={`bg-slate-900 border-2 ${borderColor} rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] w-full max-w-md overflow-hidden relative flex flex-col`}>
        ...
        <div className={`${bgColor} p-6 border-b border-white/5 text-center`}>
            ...
            <h3 className="text-xl font-black text-white uppercase tracking-tighter leading-none">
                {isRefresh ? (language === 'sk' ? 'AKTUALIZÁCIA SYSTÉMU' : 'SYSTEM UPDATE') : (language === 'sk' ? 'DÔLEŽITÁ SPRÁVA' : 'IMPORTANT NOTICE')}
            </h3>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">
                {isRefresh ? (language === 'sk' ? 'Nová verzia je k dispozícii' : 'A new version is available') : (language === 'sk' ? 'Aktualizácia stavu vašej úlohy' : 'Update on your task')}
            </p>
        </div>
        ...
        <div className="p-8 space-y-6">
            ...
            <div>
                ...
                <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                    <p className={`font-bold text-base ${isMissing ? 'text-red-400' : isRefresh ? 'text-indigo-400' : 'text-white'}`}>
                        {notification.reason}
                    </p>
                </div>
            </div>
        ...
        <button 
            ...
            className={`w-full py-5 font-black uppercase text-xs tracking-[0.2em] transition-all border-t border-slate-700 ${isRefresh ? 'bg-indigo-600 hover:bg-indigo-500 text-white' : 'bg-slate-800 hover:bg-slate-700 text-white hover:text-teal-400'}`}
        >
            {isRefresh ? (language === 'sk' ? 'OBNOVIŤ APLIKÁCIU' : 'REFRESH APP') : (language === 'sk' ? 'BERIEM NA VEDOMIE' : 'ACKNOWLEDGE')}
        </button>
      </div>
    </div>,
    document.body
