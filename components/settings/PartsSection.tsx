...
const PartsSection: React.FC<PartsSectionProps> = memo(({ parts, onAddPart, onBatchAddParts, onDeletePart, onDeleteAllParts }) => {
  ...
  const handleExport = () => {
    if (parts.length === 0) {
      alert('V databáze nie sú žiadne diely na export.');
      return;
    }
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Databáza Dielov');
    XLSX.writeFile(wb, `Databaza_Dielov_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
