...
import TempInventoriesSection from './TempInventoriesSection';
...
const Icons = {
  ...
  Inventory: () => <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
};
...
const SettingsTab: React.FC<SettingsTabProps> = (props) => {
  ...
  const navTiles = useMemo(() => {
    const all = [
      ...
      { id: 'temp_inventories', label: language === 'sk' ? 'INVENTÚRY' : 'INVENTORIES', icon: <Icons.Inventory />, perm: 'perm_tab_inventory' },
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-fade-in px-4 md:px-0">
      ...
      <div className="animate-fade-in">
        ...
        {activeSubTab === 'temp_inventories' && (
          <TempInventoriesSection resolveName={resolveName} />
        )}
